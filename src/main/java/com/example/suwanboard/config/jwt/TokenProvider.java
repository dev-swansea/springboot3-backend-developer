package com.example.suwanboard.config.jwt;
...
public class TokenProvider {
  ...
  private String makeToken(User user, Date expire) {
    ...
    return Jwts.builder()
            ...
            .setIssuer(jwtProperties.getIssuer()) // 내용 iss: suwan@naver.com(properties 파일 설정 값)
            .setIssuedAt(now) // 내용 iat(토큰 발급 시간): 현재시간
            .setExpiration(expire) // 내용 exp: expriy 멤버 변수값 (? 매개변수{지역변수} 아님?)
  ...
  public boolean validToken(String token) {
    try {
      ...
      return true;
    } catch (Exception e) { // 복호화시 에러가 발생하면 유효하지 않은 토큰
      return false;
    }
  }
